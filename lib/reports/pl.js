// P&L computation helpers

// mode: 'detailed' | 'non_detailed' | 'full'
//   detailed     — individual CC line items + direct bank items (no autopayments)
//   non_detailed — bank transactions only (includes autopayments, ties to bank balance)
//   full         — everything

// expenseFilter: 'all' | 'opex_only' | 'exclude_capex'
//   all          — include everything
//   opex_only    — only opex transactions (for bank/deal room P&L)
//   exclude_capex — exclude capex (CapEx goes to balance sheet)

export function filterTransactions(transactions, mode) {
  if (mode === 'detailed') {
    // Non-autopayment items only — CC individual charges + direct bank expenses (no lump-sum payments)
    return transactions.filter(t =>
      !t.is_autopayment &&
      (t.source_type === 'credit_card' || t.source_type === 'bank')
    )
  }
  if (mode === 'non_detailed') {
    // Bank only (includes autopayments — these tie to monthly bank balances)
    return transactions.filter(t => t.source_type === 'bank')
  }
  // full — everything
  return transactions
}

export function applyExpenseFilter(transactions, categories, expenseFilter) {
  if (expenseFilter === 'opex_only') {
    const categoryMap = {}
    categories.forEach(c => { categoryMap[c.id] = c })
    return transactions.filter(t => {
      const cat = t.category_id ? categoryMap[t.category_id] : null
      const catType = cat ? cat.type : (t.amount < 0 ? 'expense' : 'income')
      // Always keep income-category transactions (positive revenue or reversals)
      if (catType === 'income') return true
      // For expense-category transactions (including refunds), require opex tag
      return t.expense_type === 'opex'
    })
  }
  if (expenseFilter === 'exclude_capex') {
    return transactions.filter(t => t.expense_type !== 'capex')
  }
  return transactions
}

export function buildPL(transactions, categories) {
  const categoryMap = {}
  categories.forEach(c => { categoryMap[c.id] = c })

  const income = {}
  const expenses = {}
  let totalIncome = 0
  let totalExpenses = 0

  transactions.forEach(t => {
    const cat = t.category_id ? categoryMap[t.category_id] : null
    const catName = cat ? cat.name : 'Uncategorized'
    const catType = cat ? cat.type : (t.amount < 0 ? 'expense' : 'income')

    if (catType === 'income') {
      // Income category: positive = revenue, negative = reversal (reduces income)
      income[catName] = (income[catName] || 0) + t.amount
      totalIncome += t.amount
    } else {
      // Expense category: negative = expense, positive = refund (contra-expense, reduces total)
      const contribution = t.amount < 0 ? Math.abs(t.amount) : -Math.abs(t.amount)
      expenses[catName] = (expenses[catName] || 0) + contribution
      totalExpenses += contribution
    }
  })

  const noi = totalIncome - totalExpenses

  return {
    income: Object.entries(income).sort((a, b) => b[1] - a[1]),
    expenses: Object.entries(expenses).sort((a, b) => b[1] - a[1]),
    totalIncome,
    totalExpenses,
    noi,
  }
}

export function groupByMonth(transactions) {
  const months = {}
  transactions.forEach(t => {
    const key = t.date.slice(0, 7) // YYYY-MM
    if (!months[key]) months[key] = []
    months[key].push(t)
  })
  return Object.entries(months).sort(([a], [b]) => a.localeCompare(b))
}

export function formatCurrency(amount) {
  return new Intl.NumberFormat('en-US', {
    style: 'currency',
    currency: 'USD',
    minimumFractionDigits: 2,
  }).format(amount)
}
